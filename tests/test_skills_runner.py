...
intermediate tool calls or deliberation. Everything here is async - see
skills/runner.py's module docstring for why.
"""
...
class TestSkillRunnerWorker:
    ...
    async def test_runs_to_completion_and_returns_a_result_envelope(self, tmp_path):
        worker = self.make_worker(tmp_path, [submit_result_response(summary="Found the missing header.")])

        reply = await worker.handle(delegate_skill_call("cmake.diagnose_configure"))
    ...
    async def test_only_the_result_crosses_back_nothing_else(self, tmp_path):
    ...
    async def test_unknown_skill_returns_not_found_error(self, tmp_path):
        worker = self.make_worker(tmp_path, [])
        reply = await worker.handle(delegate_skill_call("nope.nothing"))
        assert isinstance(reply, ErrorEnvelope)
        assert reply.payload.code == "not_found"

    async def test_missing_name_returns_invalid_args_error(self, tmp_path):
        worker = self.make_worker(tmp_path, [])
        call = CallEnvelope(source="repl.master", target="skill.runner", payload=CallPayload(action="delegate_skill", args={"brief": "x"}))
        reply = await worker.handle(call)
        assert isinstance(reply, ErrorEnvelope)
        assert reply.payload.code == "invalid_args"

    async def test_unsupported_action_returns_error(self, tmp_path):
        worker = self.make_worker(tmp_path, [])
    ...
    async def test_provider_factory_is_given_the_skills_model_tier(self, tmp_path):
        ...
        await worker.handle(delegate_skill_call("git.status_check"))

        assert seen_tiers == ["smart"]

    async def test_inputs_are_included_in_the_brief(self, tmp_path):
        captured_prompts = []
        ...
        await worker.handle(delegate_skill_call("cmake.diagnose_configure", brief="diagnose it", inputs={"repo_dir": "/src/myrepo"}))

        first_prompt = captured_prompts[0]
        assert any("/src/myrepo" in (m.content or "") for m in first_prompt)

    async def test_dispatch_is_used_for_bus_tools_inside_the_delegated_run(self, tmp_path):
        ...
        await worker.handle(delegate_skill_call("cmake.diagnose_configure"))
...
class TestAskPassthrough:
    ...
    async def test_call_that_needs_confirmation_returns_ask_not_result(self, tmp_path):
    ...
    async def test_answering_yes_dispatches_and_completes(self, tmp_path):
        dispatcher_calls = []
        ...
        ask = await worker.handle(delegate_skill_call("cmake.diagnose_configure"))
        ...
        reply = await worker.handle(answer)

        assert isinstance(reply, ResultEnvelope)
        assert reply.payload.summary == "cleaned"
        assert len(dispatcher_calls) == 1

    async def test_answering_no_declines_and_completes(self, tmp_path):
    ...
    async def test_unknown_resume_token_returns_a_clear_error(self, tmp_path):
        worker = self.make_worker(tmp_path, [])
    ...
    async def test_the_result_still_only_carries_the_structured_payload(self, tmp_path):
        async def dispatch(call):
    ...
    async def test_resume_driven_from_a_different_task_than_the_one_that_suspended(self, tmp_path):
        """The same property proven at the AgentLoop level in
        test_agent_loop.py, here through the actual worker: an ANSWER
        handled by a different asyncio Task than the one that produced
        the suspension (the real shape once two separate Repliers' worker
        pools are both scheduling Tasks on one event loop) still resumes
        the right run."""
        ...
        async def answer_elsewhere():
            return await worker.handle(answer)

        reply = await asyncio.ensure_future(answer_elsewhere())
...
class TestCancel:
    ...
    async def test_cancel_before_the_call_starts_aborts_it_immediately(self, tmp_path):
        ...
        cancel_reply = await worker.handle_cancel(
    ...
    async def test_cancel_for_an_unknown_span_id_is_a_harmless_no_op(self, tmp_path):
        worker = self.make_worker(tmp_path, [])
        reply = await worker.handle_cancel(
    ...
    async def test_served_over_nng_end_to_end(self, tmp_path, unique_name):
        ...
        with Replier(addr) as rep:
            ...
            with Requester(addr) as req:
                reply = await req.call(delegate_skill_call("cmake.diagnose_configure"))
            assert isinstance(reply, ResultEnvelope)
