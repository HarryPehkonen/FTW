...
- **A real Ctrl-C trigger for CANCEL.** The CANCEL protocol itself is built and tested (Phase 3: a control channel, per-span_id cancel flags, cooperative checking between a delegated run's steps) — what's missing is Ctrl-C reliably *reaching* it. Confirmed empirically: pynng's blocking `send()`/`recv()` don't release the GIL, so no thread gets a chance to process a pending SIGINT until the call itself returns, and a naive short-poll retry loop hits a separate pynng bug (`BadState`) retrying `recv()` on the same `Req0` socket after a timeout. A real fix means resend- and idempotency-key-based retry in `bus.Requester.call()` (it disables resend outright today, for the correctness reasons in its own docstring) — its own piece of work, tracked by the `xfail`ed `tests/test_bus.py::TestRecvIsInterruptible`.
