...
### Technical debt (audited for on request, not from the review)

Two places where already-shipped code doesn't keep a promise it appears to make — as distinct from everything above, which is unbuilt features, not debt. Both were considered for an immediate fix and deliberately left alone instead — see the reasoning below each.

- **`ContextWorkbench.on_turn_evicted` is built but never wired up — but this is a consolidation gap, not data loss.** The workbench takes a callback meant to flush aged-out turns to episodic memory before they leave the Turn Horizon (`workbench.py:102,214-215`); nothing in `repl/cli.py` or `skills/runner.py` passes one. Checked before assuming the worst: every step of every turn is *already* durably traced regardless — `cli.py`'s `on_event` (line 216) writes every `event.model.request`/`event.model.response`/`event.tool.result` to `TraceWriter` as it happens, so an evicted turn's raw material already sits in `traces/`, recoverable. What the hook would add is a *consolidated* single record of one evicted turn, in whatever shape Phase 5's episodic retrieval ends up wanting (raw messages? summarized? chunked for BM25?) — a shape nobody's designed yet. Wiring it now means guessing that shape ahead of Phase 5's actual design and likely rebuilding it later. Left for when Phase 5 is real.
- **`idempotency_key` is fully built and never set by any caller — and setting it today would be inert, not a real fix.** Checked for a retry path before doing anything: there isn't one anywhere in this codebase. No caller ever resends a call. Setting `idempotency_key` on an outgoing `CallEnvelope` today would change nothing observable — the dedupe cache would see each key exactly once, same as no key at all — and would be actively misleading (looks like a safety net that isn't exercised by anything). Doing this properly means designing retry policy first (what's retryable, backoff, which layer owns it), which is its own unscoped feature, not a one-line fix. Left alone until retry logic is a real, designed thing.

### Superseding a redundant Turn Horizon exchange

Came up wanting `find_skill` → `mount_skill` to collapse in context: once a skill is mounted, the `find_skill` call/result pair that led to it is redundant — the same information (name + description) is now sitting in `mount_skill`'s own result (fixed: both tool results include the description now, see git history). Splitting this in two mattered: adding the description to each result was a one-line fix, already done. Actually *removing* the now-redundant `find_skill` exchange from the live Turn Horizon is a different, unbuilt kind of operation — everything today either stays until it ages out by FIFO, or gets evicted as a whole frame's subtree on unmount. There's no "this earlier exchange is now redundant, drop it" mechanism, and building one means either fragile pattern-matching at render time or a real "supersede" concept in `ContextWorkbench` — which would generalize beyond this one case (a `read_output` call superseded by a later one on the same handle; an old `pin` overwritten by a new one on the same key). Also a real tradeoff to weigh, not just cost: the search step is a "why this skill and not another candidate" trail: dropping it from the live prompt loses that from the model's working view (still recoverable from `traces/`, per the `on_turn_evicted` item above). Left unbuilt; revisit if Turn Horizon pressure from this specific pattern ever actually bites.
